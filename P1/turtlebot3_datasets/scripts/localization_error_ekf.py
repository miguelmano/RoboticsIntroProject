...
class EKFLocalizationErrorPlotter:
    def __init__(self):
        rospy.init_node('ekf_localization_error_plotter', anonymous=True)
        ...
        self.ekf_sub = Subscriber('/odometry/filtered', Odometry)  # EKF output topic
        self.gt_sub = Subscriber('/ground_truth_path', Path)        # Ground truth path

        # Synchronize EKF and ground truth messages
        self.ts = ApproximateTimeSynchronizer([self.ekf_sub, self.gt_sub], queue_size=50, slop=1.0)
        ...
        self.animation = FuncAnimation(self.fig, self.update_plot, interval=500)  # Update every 0.5 seconds

        rospy.loginfo("EKF Localization Error Plotter Initialized")

    def sync_callback(self, ekf_msg, gt_msg):
        rospy.loginfo("Receiving synchronized data.")  # Debugging: check if data is being received

        # Store the latest EKF pose
        self.ekf_pose = ekf_msg.pose.pose

        # Calculate uncertainty from the covariance matrix in the EKF's odometry message
        covariance = np.array(ekf_msg.pose.covariance).reshape(6, 6)
    ...
    def compute_error(self):
        # Compute the error between EKF pose and ground truth pose
        error_x = self.ekf_pose.position.x - self.ground_truth_pose.position.x
        error_y = self.ekf_pose.position.y - self.ground_truth_pose.position.y
...
if __name__ == '__main__':
    plotter = EKFLocalizationErrorPlotter()
